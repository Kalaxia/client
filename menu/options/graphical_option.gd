...
func _on_resize_window():
	if is_resizable:
		Config.set_config_windows_resolution(OS.window_size)
	_check_option_state()
...
func _on_fullscreen_pressed(is_fullscreen):
	if is_fullscreen:
		...
		Config.set_config_windows_maximized(false)
	OS.window_fullscreen = is_fullscreen
	Config.set_config_windows_fullscreen(is_fullscreen)
	if ! is_fullscreen:
		OS.set_window_size(Config.get_windows_resolution())
	_check_option_state()
...
func _on_maximize_pressed(is_maximize):
	if is_maximize:
		is_resizable = false
		Utils.set_windows_resizable(false)
	Config.set_config_windows_maximized(is_maximize)
...
func _on_screen_changed(screen_id):
	Config.set_config_windows_screen(screen_id)
