...
func update_elements():
	...
	$PanelContainer/hBoxContainer/ShipNumber.text = ship_queue.quantity as String
	$PanelContainer/hBoxContainer/ShipModel.text = tr("ship." + ship_queue.model) 
	update_time()
...
func update_time():
	...
	$PanelContainer/hBoxContainer/TimeRemaining.text = tr("%d s") % floor(max(time_remaining, 0) / 1000.0 )
