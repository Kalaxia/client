...
func update_price():
	$PanelContainer/HBoxContainer/Price.text = String(Utils.SHIP_PRICES[ship_model] * $PanelContainer/HBoxContainer/SpinBox.value)
	update_order_button_state()

func update_elements():
	if ship_model != null:
		$PanelContainer/HBoxContainer/TextureRect.texture = Utils.TEXTURE_SHIP_MODEL[ship_model]
		$PanelContainer/HBoxContainer/Label.text = tr("ship." + ship_model)
